...
description: 'Run the plan-driven implement → review → fix → summary loop. Use after a plan is approved (authored with Fable 5.1 high in Plan mode) to implement it in a dev subagent, review with a Grok 4.6 high fast subagent (/code-review), fix findings, then summarize. If the first review reports ≥1 Critical issue, run one extra review+fix round after those fixes.'
---
...
1. An approved plan exists (from Plan mode / this conversation; plans should be authored with Fable 5.1 high). If not, stop and ask for one.
