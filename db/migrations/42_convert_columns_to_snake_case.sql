...
ALTER TABLE `history_groups_attempts`
	...
	RENAME COLUMN `sAllLangProg` TO `all_lang_prog`,
	RENAME COLUMN `iVersion` TO `version`,
	RENAME COLUMN `sPropagationState` TO `propagation_state`,
	RENAME COLUMN `iNextVersion` TO `next_version`,
	RENAME COLUMN `bDeleted` TO `deleted`,
	RENAME INDEX `iVersion` TO `version`,
	RENAME INDEX `sAncestorsComputationState` TO `ancestors_computation_state`,
...
ALTER TABLE `history_items`
	...
	RENAME COLUMN `sAncestorsComputationState` TO `ancestors_computation_state`,
	RENAME COLUMN `sAncestorsAccessComputationState` TO `ancestors_access_computation_state`,
	RENAME COLUMN `iVersion` TO `version`,
	RENAME COLUMN `iNextVersion` TO `next_version`,
	RENAME COLUMN `bDeleted` TO `deleted`,
	RENAME INDEX `ID` TO `id`,
...
ALTER TABLE `history_messages`
	...
	RENAME COLUMN `bReadByCandidate` TO `read_by_candidate`,
	RENAME COLUMN `iVersion` TO `version`,
	RENAME COLUMN `iNextVersion` TO `next_version`,
	RENAME COLUMN `bDeleted` TO `deleted`,
	RENAME INDEX `iVersion` TO `version`,
...
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_groups_attempts` BEFORE INSERT ON `groups_attempts` FOR EACH ROW BEGIN IF (NEW.id IS NULL OR NEW.id = 0) THEN SET NEW.id = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.version = @curVersion; SET NEW.minus_score = -NEW.score; END
-- +migrate StatementEnd
...
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_update_groups_groups` BEFORE UPDATE ON `groups_groups` FOR EACH ROW BEGIN
...
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `after_delete_groups_items` AFTER DELETE ON `groups_items` FOR EACH ROW BEGIN DELETE FROM groups_items_propagate where id = OLD.id ; END
-- +migrate StatementEnd
DROP TRIGGER `before_insert_groups_login_prefixes`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_groups_login_prefixes` BEFORE INSERT ON `groups_login_prefixes` FOR EACH ROW BEGIN IF (NEW.id IS NULL OR NEW.id = 0) THEN SET NEW.id = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.version = @curVersion; END
-- +migrate StatementEnd
DROP TRIGGER `after_insert_groups_login_prefixes`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `after_insert_groups_login_prefixes` AFTER INSERT ON `groups_login_prefixes` FOR EACH ROW BEGIN INSERT INTO `history_groups_login_prefixes` (`id`,`version`,`group_id`,`prefix`) VALUES (NEW.`id`,@curVersion,NEW.`group_id`,NEW.`prefix`); END
-- +migrate StatementEnd
DROP TRIGGER `before_update_groups_login_prefixes`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_update_groups_login_prefixes` BEFORE UPDATE ON `groups_login_prefixes` FOR EACH ROW BEGIN IF NEW.version <> OLD.version THEN SET @curVersion = NEW.version; ELSE SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion; END IF; IF NOT (OLD.`id` = NEW.`id` AND OLD.`group_id` <=> NEW.`group_id` AND OLD.`prefix` <=> NEW.`prefix`) THEN   SET NEW.version = @curVersion;   UPDATE `history_groups_login_prefixes` SET `next_version` = @curVersion WHERE `id` = OLD.`id` AND `next_version` IS NULL;   INSERT INTO `history_groups_login_prefixes` (`id`,`version`,`group_id`,`prefix`)       VALUES (NEW.`id`,@curVersion,NEW.`group_id`,NEW.`prefix`) ; END IF; END
-- +migrate StatementEnd
DROP TRIGGER `before_delete_groups_login_prefixes`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_delete_groups_login_prefixes` BEFORE DELETE ON `groups_login_prefixes` FOR EACH ROW BEGIN SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion; UPDATE `history_groups_login_prefixes` SET `next_version` = @curVersion WHERE `id` = OLD.`id` AND `next_version` IS NULL; INSERT INTO `history_groups_login_prefixes` (`id`,`version`,`group_id`,`prefix`, `deleted`) VALUES (OLD.`id`,@curVersion,OLD.`group_id`,OLD.`prefix`, 1); END
-- +migrate StatementEnd
DROP TRIGGER `before_insert_items`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_items` BEFORE INSERT ON `items` FOR EACH ROW BEGIN IF (NEW.id IS NULL OR NEW.id = 0) THEN SET NEW.id = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.version = @curVersion; SELECT platforms.id INTO @platformID FROM platforms WHERE NEW.url REGEXP platforms.regexp ORDER BY platforms.priority DESC LIMIT 1 ; SET NEW.platform_id=@platformID ; END
-- +migrate StatementEnd
...
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `after_delete_items` AFTER DELETE ON `items` FOR EACH ROW BEGIN DELETE FROM items_propagate where id = OLD.id ; END
-- +migrate StatementEnd
DROP TRIGGER `before_insert_items_ancestors`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_items_ancestors` BEFORE INSERT ON `items_ancestors` FOR EACH ROW BEGIN IF (NEW.id IS NULL OR NEW.id = 0) THEN SET NEW.id = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.version = @curVersion; END
-- +migrate StatementEnd
...
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_items_strings` BEFORE INSERT ON `items_strings` FOR EACH ROW BEGIN IF (NEW.id IS NULL OR NEW.id = 0) THEN SET NEW.id = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.version = @curVersion; END
-- +migrate StatementEnd
DROP TRIGGER `after_insert_items_strings`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `after_insert_items_strings` AFTER INSERT ON `items_strings` FOR EACH ROW BEGIN INSERT INTO `history_items_strings` (`id`,`version`,`item_id`,`language_id`,`translator`,`title`,`image_url`,`subtitle`,`description`,`edu_comment`,`ranking_comment`) VALUES (NEW.`id`,@curVersion,NEW.`item_id`,NEW.`language_id`,NEW.`translator`,NEW.`title`,NEW.`image_url`,NEW.`subtitle`,NEW.`description`,NEW.`edu_comment`,NEW.`ranking_comment`); END
-- +migrate StatementEnd
DROP TRIGGER `before_update_items_strings`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_update_items_strings` BEFORE UPDATE ON `items_strings` FOR EACH ROW BEGIN IF NEW.version <> OLD.version THEN SET @curVersion = NEW.version; ELSE SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion; END IF; IF NOT (OLD.`id` = NEW.`id` AND OLD.`item_id` <=> NEW.`item_id` AND OLD.`language_id` <=> NEW.`language_id` AND OLD.`translator` <=> NEW.`translator` AND OLD.`title` <=> NEW.`title` AND OLD.`image_url` <=> NEW.`image_url` AND OLD.`subtitle` <=> NEW.`subtitle` AND OLD.`description` <=> NEW.`description` AND OLD.`edu_comment` <=> NEW.`edu_comment` AND OLD.`ranking_comment` <=> NEW.`ranking_comment`) THEN   SET NEW.version = @curVersion;   UPDATE `history_items_strings` SET `next_version` = @curVersion WHERE `id` = OLD.`id` AND `next_version` IS NULL;   INSERT INTO `history_items_strings` (`id`,`version`,`item_id`,`language_id`,`translator`,`title`,`image_url`,`subtitle`,`description`,`edu_comment`,`ranking_comment`)       VALUES (NEW.`id`,@curVersion,NEW.`item_id`,NEW.`language_id`,NEW.`translator`,NEW.`title`,NEW.`image_url`,NEW.`subtitle`,NEW.`description`,NEW.`edu_comment`,NEW.`ranking_comment`) ; END IF; END
-- +migrate StatementEnd
DROP TRIGGER `before_delete_items_strings`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_delete_items_strings` BEFORE DELETE ON `items_strings` FOR EACH ROW BEGIN SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion; UPDATE `history_items_strings` SET `next_version` = @curVersion WHERE `id` = OLD.`id` AND `next_version` IS NULL; INSERT INTO `history_items_strings` (`id`,`version`,`item_id`,`language_id`,`translator`,`title`,`image_url`,`subtitle`,`description`,`edu_comment`,`ranking_comment`, `deleted`) VALUES (OLD.`id`,@curVersion,OLD.`item_id`,OLD.`language_id`,OLD.`translator`,OLD.`title`,OLD.`image_url`,OLD.`subtitle`,OLD.`description`,OLD.`edu_comment`,OLD.`ranking_comment`, 1); END
-- +migrate StatementEnd
DROP TRIGGER `before_insert_languages`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_languages` BEFORE INSERT ON `languages` FOR EACH ROW BEGIN IF (NEW.id IS NULL OR NEW.id = 0) THEN SET NEW.id = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.version = @curVersion; END
-- +migrate StatementEnd
DROP TRIGGER `after_insert_languages`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `after_insert_languages` AFTER INSERT ON `languages` FOR EACH ROW BEGIN INSERT INTO `history_languages` (`id`,`version`,`name`,`code`) VALUES (NEW.`id`,@curVersion,NEW.`name`,NEW.`code`); END
-- +migrate StatementEnd
DROP TRIGGER `before_update_languages`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_update_languages` BEFORE UPDATE ON `languages` FOR EACH ROW BEGIN IF NEW.version <> OLD.version THEN SET @curVersion = NEW.version; ELSE SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion; END IF; IF NOT (OLD.`id` = NEW.`id` AND OLD.`name` <=> NEW.`name` AND OLD.`code` <=> NEW.`code`) THEN   SET NEW.version = @curVersion;   UPDATE `history_languages` SET `next_version` = @curVersion WHERE `id` = OLD.`id` AND `next_version` IS NULL;   INSERT INTO `history_languages` (`id`,`version`,`name`,`code`)       VALUES (NEW.`id`,@curVersion,NEW.`name`,NEW.`code`) ; END IF; END
-- +migrate StatementEnd
DROP TRIGGER `before_delete_languages`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_delete_languages` BEFORE DELETE ON `languages` FOR EACH ROW BEGIN SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion; UPDATE `history_languages` SET `next_version` = @curVersion WHERE `id` = OLD.`id` AND `next_version` IS NULL; INSERT INTO `history_languages` (`id`,`version`,`name`,`code`, `deleted`) VALUES (OLD.`id`,@curVersion,OLD.`name`,OLD.`code`, 1); END
-- +migrate StatementEnd
DROP TRIGGER `before_insert_messages`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_messages` BEFORE INSERT ON `messages` FOR EACH ROW BEGIN IF (NEW.id IS NULL OR NEW.id = 0) THEN SET NEW.id = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.version = @curVersion; END
-- +migrate StatementEnd
DROP TRIGGER `after_insert_messages`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `after_insert_messages` AFTER INSERT ON `messages` FOR EACH ROW BEGIN INSERT INTO `history_messages` (`id`,`version`,`thread_id`,`user_id`,`submission_date`,`published`,`title`,`body`,`trainers_only`,`archived`,`persistant`) VALUES (NEW.`id`,@curVersion,NEW.`thread_id`,NEW.`user_id`,NEW.`submission_date`,NEW.`published`,NEW.`title`,NEW.`body`,NEW.`trainers_only`,NEW.`archived`,NEW.`persistant`); END
-- +migrate StatementEnd
DROP TRIGGER `before_update_messages`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_update_messages` BEFORE UPDATE ON `messages` FOR EACH ROW BEGIN IF NEW.version <> OLD.version THEN SET @curVersion = NEW.version; ELSE SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion; END IF; IF NOT (OLD.`id` = NEW.`id` AND OLD.`thread_id` <=> NEW.`thread_id` AND OLD.`user_id` <=> NEW.`user_id` AND OLD.`submission_date` <=> NEW.`submission_date` AND OLD.`published` <=> NEW.`published` AND OLD.`title` <=> NEW.`title` AND OLD.`body` <=> NEW.`body` AND OLD.`trainers_only` <=> NEW.`trainers_only` AND OLD.`archived` <=> NEW.`archived` AND OLD.`persistant` <=> NEW.`persistant`) THEN   SET NEW.version = @curVersion;   UPDATE `history_messages` SET `next_version` = @curVersion WHERE `id` = OLD.`id` AND `next_version` IS NULL;   INSERT INTO `history_messages` (`id`,`version`,`thread_id`,`user_id`,`submission_date`,`published`,`title`,`body`,`trainers_only`,`archived`,`persistant`)       VALUES (NEW.`id`,@curVersion,NEW.`thread_id`,NEW.`user_id`,NEW.`submission_date`,NEW.`published`,NEW.`title`,NEW.`body`,NEW.`trainers_only`,NEW.`archived`,NEW.`persistant`) ; END IF; END
-- +migrate StatementEnd
DROP TRIGGER `before_delete_messages`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_delete_messages` BEFORE DELETE ON `messages` FOR EACH ROW BEGIN SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion; UPDATE `history_messages` SET `next_version` = @curVersion WHERE `id` = OLD.`id` AND `next_version` IS NULL; INSERT INTO `history_messages` (`id`,`version`,`thread_id`,`user_id`,`submission_date`,`published`,`title`,`body`,`trainers_only`,`archived`,`persistant`, `deleted`) VALUES (OLD.`id`,@curVersion,OLD.`thread_id`,OLD.`user_id`,OLD.`submission_date`,OLD.`published`,OLD.`title`,OLD.`body`,OLD.`trainers_only`,OLD.`archived`,OLD.`persistant`, 1); END
-- +migrate StatementEnd
DROP TRIGGER `before_insert_threads`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_threads` BEFORE INSERT ON `threads` FOR EACH ROW BEGIN IF (NEW.id IS NULL OR NEW.id = 0) THEN SET NEW.id = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.version = @curVersion; END
-- +migrate StatementEnd
...
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_users` BEFORE INSERT ON `users` FOR EACH ROW BEGIN IF (NEW.id IS NULL OR NEW.id = 0) THEN SET NEW.id = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.version = @curVersion; END
-- +migrate StatementEnd
...
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_users_items` BEFORE INSERT ON `users_items` FOR EACH ROW BEGIN IF (NEW.id IS NULL OR NEW.id = 0) THEN SET NEW.id = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.version = @curVersion; END
-- +migrate StatementEnd
...
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_users_threads` BEFORE INSERT ON `users_threads` FOR EACH ROW BEGIN IF (NEW.id IS NULL OR NEW.id = 0) THEN SET NEW.id = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.version = @curVersion; END
-- +migrate StatementEnd
DROP TRIGGER `after_insert_users_threads`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `after_insert_users_threads` AFTER INSERT ON `users_threads` FOR EACH ROW BEGIN INSERT INTO `history_users_threads` (`id`,`version`,`user_id`,`thread_id`,`last_read_date`,`last_write_date`,`starred`) VALUES (NEW.`id`,@curVersion,NEW.`user_id`,NEW.`thread_id`,NEW.`last_read_date`,NEW.`last_write_date`,NEW.`starred`); END
-- +migrate StatementEnd
DROP TRIGGER `before_update_users_threads`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_update_users_threads` BEFORE UPDATE ON `users_threads` FOR EACH ROW BEGIN IF NEW.version <> OLD.version THEN SET @curVersion = NEW.version; ELSE SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion; END IF; IF NOT (OLD.`id` = NEW.`id` AND OLD.`user_id` <=> NEW.`user_id` AND OLD.`thread_id` <=> NEW.`thread_id` AND OLD.`last_read_date` <=> NEW.`last_read_date` AND OLD.`last_write_date` <=> NEW.`last_write_date` AND OLD.`starred` <=> NEW.`starred`) THEN   SET NEW.version = @curVersion;   UPDATE `history_users_threads` SET `next_version` = @curVersion WHERE `id` = OLD.`id` AND `next_version` IS NULL;   INSERT INTO `history_users_threads` (`id`,`version`,`user_id`,`thread_id`,`last_read_date`,`last_write_date`,`starred`)       VALUES (NEW.`id`,@curVersion,NEW.`user_id`,NEW.`thread_id`,NEW.`last_read_date`,NEW.`last_write_date`,NEW.`starred`) ; END IF; END
-- +migrate StatementEnd
DROP TRIGGER `before_delete_users_threads`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_delete_users_threads` BEFORE DELETE ON `users_threads` FOR EACH ROW BEGIN SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion; UPDATE `history_users_threads` SET `next_version` = @curVersion WHERE `id` = OLD.`id` AND `next_version` IS NULL; INSERT INTO `history_users_threads` (`id`,`version`,`user_id`,`thread_id`,`last_read_date`,`last_write_date`,`starred`, `deleted`) VALUES (OLD.`id`,@curVersion,OLD.`user_id`,OLD.`thread_id`,OLD.`last_read_date`,OLD.`last_write_date`,OLD.`starred`, 1); END
-- +migrate StatementEnd
...
ALTER TABLE `history_groups_attempts`
	...
	RENAME COLUMN `all_lang_prog` TO `sAllLangProg`,
	RENAME COLUMN `version` TO `iVersion`,
	RENAME COLUMN `propagation_state` TO `sPropagationState`,
	RENAME COLUMN `next_version` TO `iNextVersion`,
	RENAME COLUMN `deleted` TO `bDeleted`,
	RENAME INDEX `version` TO `iVersion`,
	RENAME INDEX `ancestors_computation_state` TO `sAncestorsComputationState`,
...
ALTER TABLE `history_items`
	...
	RENAME COLUMN `ancestors_computation_state` TO `sAncestorsComputationState`,
	RENAME COLUMN `ancestors_access_computation_state` TO `sAncestorsAccessComputationState`,
	RENAME COLUMN `version` TO `iVersion`,
	RENAME COLUMN `next_version` TO `iNextVersion`,
	RENAME COLUMN `deleted` TO `bDeleted`,
	RENAME INDEX `id` TO `ID`,
...
ALTER TABLE `history_messages`
	...
	RENAME COLUMN `read_by_candidate` TO `bReadByCandidate`,
	RENAME COLUMN `version` TO `iVersion`,
	RENAME COLUMN `next_version` TO `iNextVersion`,
	RENAME COLUMN `deleted` TO `bDeleted`,
	RENAME INDEX `version` TO `iVersion`,
...
DROP TRIGGER `before_insert_filters`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_filters` BEFORE INSERT ON `filters` FOR EACH ROW BEGIN IF (NEW.ID IS NULL OR NEW.ID = 0) THEN SET NEW.ID = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.iVersion = @curVersion; END
-- +migrate StatementEnd
DROP TRIGGER `after_insert_filters`;
-- +migrate StatementBegin
...
DROP TRIGGER `before_insert_groups`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_groups` BEFORE INSERT ON `groups` FOR EACH ROW BEGIN IF (NEW.ID IS NULL OR NEW.ID = 0) THEN SET NEW.ID = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.iVersion = @curVersion; END
-- +migrate StatementEnd
DROP TRIGGER `after_insert_groups`;
-- +migrate StatementBegin
...
DROP TRIGGER `before_insert_groups_ancestors`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_groups_ancestors` BEFORE INSERT ON `groups_ancestors` FOR EACH ROW BEGIN IF (NEW.ID IS NULL OR NEW.ID = 0) THEN SET NEW.ID = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.iVersion = @curVersion; END
-- +migrate StatementEnd
DROP TRIGGER `after_insert_groups_ancestors`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `after_insert_groups_ancestors` AFTER INSERT ON `groups_ancestors` FOR EACH ROW BEGIN INSERT INTO `history_groups_ancestors` (`ID`,`iVersion`,`idGroupAncestor`,`idGroupChild`,`bIsSelf`) VALUES (NEW.`ID`,@curVersion,NEW.`idGroupAncestor`,NEW.`idGroupChild`,NEW.`bIsSelf`); END
-- +migrate StatementEnd
DROP TRIGGER `before_update_groups_ancestors`;
-- +migrate StatementBegin
...
DROP TRIGGER `before_insert_groups_attempts`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_groups_attempts` BEFORE INSERT ON `groups_attempts` FOR EACH ROW BEGIN IF (NEW.ID IS NULL OR NEW.ID = 0) THEN SET NEW.ID = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.iVersion = @curVersion; SET NEW.iMinusScore = -NEW.iScore; END
-- +migrate StatementEnd
DROP TRIGGER `after_insert_groups_attempts`;
-- +migrate StatementBegin
...
DROP TRIGGER `before_insert_groups_items`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_groups_items` BEFORE INSERT ON `groups_items` FOR EACH ROW BEGIN IF (NEW.ID IS NULL OR NEW.ID = 0) THEN SET NEW.ID = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.iVersion = @curVersion; SET NEW.`sPropagateAccess`='self' ; END
-- +migrate StatementEnd
DROP TRIGGER `after_insert_groups_items`;
-- +migrate StatementBegin
...
DROP TRIGGER `before_insert_groups_login_prefixes`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_groups_login_prefixes` BEFORE INSERT ON `groups_login_prefixes` FOR EACH ROW BEGIN IF (NEW.ID IS NULL OR NEW.ID = 0) THEN SET NEW.ID = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.iVersion = @curVersion; END
-- +migrate StatementEnd
DROP TRIGGER `after_insert_groups_login_prefixes`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `after_insert_groups_login_prefixes` AFTER INSERT ON `groups_login_prefixes` FOR EACH ROW BEGIN INSERT INTO `history_groups_login_prefixes` (`ID`,`iVersion`,`idGroup`,`prefix`) VALUES (NEW.`ID`,@curVersion,NEW.`idGroup`,NEW.`prefix`); END
-- +migrate StatementEnd
DROP TRIGGER `before_update_groups_login_prefixes`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_update_groups_login_prefixes` BEFORE UPDATE ON `groups_login_prefixes` FOR EACH ROW BEGIN IF NEW.iVersion <> OLD.iVersion THEN SET @curVersion = NEW.iVersion; ELSE SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion; END IF; IF NOT (OLD.`ID` = NEW.`ID` AND OLD.`idGroup` <=> NEW.`idGroup` AND OLD.`prefix` <=> NEW.`prefix`) THEN   SET NEW.iVersion = @curVersion;   UPDATE `history_groups_login_prefixes` SET `iNextVersion` = @curVersion WHERE `ID` = OLD.`ID` AND `iNextVersion` IS NULL;   INSERT INTO `history_groups_login_prefixes` (`ID`,`iVersion`,`idGroup`,`prefix`)       VALUES (NEW.`ID`,@curVersion,NEW.`idGroup`,NEW.`prefix`) ; END IF; END
-- +migrate StatementEnd
DROP TRIGGER `before_delete_groups_login_prefixes`;
-- +migrate StatementBegin
...
DROP TRIGGER `before_insert_items`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_items` BEFORE INSERT ON `items` FOR EACH ROW BEGIN IF (NEW.ID IS NULL OR NEW.ID = 0) THEN SET NEW.ID = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.iVersion = @curVersion; SELECT platforms.ID INTO @platformID FROM platforms WHERE NEW.sUrl REGEXP platforms.sRegexp ORDER BY platforms.iPriority DESC LIMIT 1 ; SET NEW.idPlatform=@platformID ; END
-- +migrate StatementEnd
DROP TRIGGER `after_insert_items`;
-- +migrate StatementBegin
...
DROP TRIGGER `before_insert_items_ancestors`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_items_ancestors` BEFORE INSERT ON `items_ancestors` FOR EACH ROW BEGIN IF (NEW.ID IS NULL OR NEW.ID = 0) THEN SET NEW.ID = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.iVersion = @curVersion; END
-- +migrate StatementEnd
DROP TRIGGER `after_insert_items_ancestors`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `after_insert_items_ancestors` AFTER INSERT ON `items_ancestors` FOR EACH ROW BEGIN INSERT INTO `history_items_ancestors` (`ID`,`iVersion`,`idItemAncestor`,`idItemChild`) VALUES (NEW.`ID`,@curVersion,NEW.`idItemAncestor`,NEW.`idItemChild`); END
-- +migrate StatementEnd
DROP TRIGGER `before_update_items_ancestors`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_update_items_ancestors` BEFORE UPDATE ON `items_ancestors` FOR EACH ROW BEGIN IF NEW.iVersion <> OLD.iVersion THEN SET @curVersion = NEW.iVersion; ELSE SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion; END IF; IF NOT (OLD.`ID` = NEW.`ID` AND OLD.`idItemAncestor` <=> NEW.`idItemAncestor` AND OLD.`idItemChild` <=> NEW.`idItemChild`) THEN   SET NEW.iVersion = @curVersion;   UPDATE `history_items_ancestors` SET `iNextVersion` = @curVersion WHERE `ID` = OLD.`ID` AND `iNextVersion` IS NULL;   INSERT INTO `history_items_ancestors` (`ID`,`iVersion`,`idItemAncestor`,`idItemChild`)       VALUES (NEW.`ID`,@curVersion,NEW.`idItemAncestor`,NEW.`idItemChild`) ; END IF; END
-- +migrate StatementEnd
DROP TRIGGER `before_delete_items_ancestors`;
-- +migrate StatementBegin
...
DROP TRIGGER `before_insert_items_items`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_items_items` BEFORE INSERT ON `items_items` FOR EACH ROW BEGIN IF (NEW.ID IS NULL OR NEW.ID = 0) THEN SET NEW.ID = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.iVersion = @curVersion; INSERT IGNORE INTO `items_propagate` (ID, sAncestorsComputationState) VALUES (NEW.idItemChild, 'todo') ON DUPLICATE KEY UPDATE `sAncestorsComputationState` = 'todo' ; END
-- +migrate StatementEnd
DROP TRIGGER `after_insert_items_items`;
-- +migrate StatementBegin
...
DROP TRIGGER `before_delete_items_items`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_delete_items_items` BEFORE DELETE ON `items_items` FOR EACH ROW BEGIN SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion; UPDATE `history_items_items` SET `iNextVersion` = @curVersion WHERE `ID` = OLD.`ID` AND `iNextVersion` IS NULL; INSERT INTO `history_items_items` (`ID`,`iVersion`,`idItemParent`,`idItemChild`,`iChildOrder`,`sCategory`,`bAccessRestricted`,`bAlwaysVisible`,`iDifficulty`, `bDeleted`) VALUES (OLD.`ID`,@curVersion,OLD.`idItemParent`,OLD.`idItemChild`,OLD.`iChildOrder`,OLD.`sCategory`,OLD.`bAccessRestricted`,OLD.`bAlwaysVisible`,OLD.`iDifficulty`, 1); INSERT IGNORE INTO `items_propagate` (`ID`, `sAncestorsComputationState`) VALUES (OLD.idItemChild, 'todo') ON DUPLICATE KEY UPDATE `sAncestorsComputationState` = 'todo'; INSERT IGNORE INTO `items_propagate` (`ID`, `sAncestorsComputationState`) VALUES (OLD.idItemParent, 'todo') ON DUPLICATE KEY UPDATE `sAncestorsComputationState` = 'todo'; INSERT IGNORE INTO `items_propagate` (`ID`, `sAncestorsComputationState`) (SELECT `items_ancestors`.`idItemChild`, 'todo' FROM `items_ancestors` WHERE `items_ancestors`.`idItemAncestor` = OLD.`idItemChild`) ON DUPLICATE KEY UPDATE `sAncestorsComputationState` = 'todo'; DELETE `items_ancestors` from `items_ancestors` WHERE `items_ancestors`.`idItemChild` = OLD.`idItemChild` and `items_ancestors`.`idItemAncestor` = OLD.`idItemParent`;DELETE `bridges` FROM `items_ancestors` `child_descendants` JOIN `items_ancestors` `parent_ancestors` JOIN `items_ancestors` `bridges` ON (`bridges`.`idItemAncestor` = `parent_ancestors`.`idItemAncestor` AND `bridges`.`idItemChild` = `child_descendants`.`idItemChild`) WHERE `parent_ancestors`.`idItemChild` = OLD.`idItemParent` AND `child_descendants`.`idItemAncestor` = OLD.`idItemChild`; DELETE `child_ancestors` FROM `items_ancestors` `child_ancestors` JOIN  `items_ancestors` `parent_ancestors` ON (`child_ancestors`.`idItemChild` = OLD.`idItemChild` AND `child_ancestors`.`idItemAncestor` = `parent_ancestors`.`idItemAncestor`) WHERE `parent_ancestors`.`idItemChild` = OLD.`idItemParent`; DELETE `parent_ancestors` FROM `items_ancestors` `parent_ancestors` JOIN  `items_ancestors` `child_ancestors` ON (`parent_ancestors`.`idItemAncestor` = OLD.`idItemParent` AND `child_ancestors`.`idItemChild` = `parent_ancestors`.`idItemChild`) WHERE `child_ancestors`.`idItemAncestor` = OLD.`idItemChild` ; INSERT IGNORE INTO `groups_items_propagate` SELECT `ID`, 'children' as `sPropagateAccess` FROM `groups_items` WHERE `groups_items`.`idItem` = OLD.`idItemParent` ON DUPLICATE KEY UPDATE sPropagateAccess='children' ; END
-- +migrate StatementEnd
DROP TRIGGER `before_insert_items_strings`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_items_strings` BEFORE INSERT ON `items_strings` FOR EACH ROW BEGIN IF (NEW.ID IS NULL OR NEW.ID = 0) THEN SET NEW.ID = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.iVersion = @curVersion; END
-- +migrate StatementEnd
DROP TRIGGER `after_insert_items_strings`;
-- +migrate StatementBegin
...
DROP TRIGGER `before_update_items_strings`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_update_items_strings` BEFORE UPDATE ON `items_strings` FOR EACH ROW BEGIN IF NEW.iVersion <> OLD.iVersion THEN SET @curVersion = NEW.iVersion; ELSE SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion; END IF; IF NOT (OLD.`ID` = NEW.`ID` AND OLD.`idItem` <=> NEW.`idItem` AND OLD.`idLanguage` <=> NEW.`idLanguage` AND OLD.`sTranslator` <=> NEW.`sTranslator` AND OLD.`sTitle` <=> NEW.`sTitle` AND OLD.`sImageUrl` <=> NEW.`sImageUrl` AND OLD.`sSubtitle` <=> NEW.`sSubtitle` AND OLD.`sDescription` <=> NEW.`sDescription` AND OLD.`sEduComment` <=> NEW.`sEduComment` AND OLD.`sRankingComment` <=> NEW.`sRankingComment`) THEN   SET NEW.iVersion = @curVersion;   UPDATE `history_items_strings` SET `iNextVersion` = @curVersion WHERE `ID` = OLD.`ID` AND `iNextVersion` IS NULL;   INSERT INTO `history_items_strings` (`ID`,`iVersion`,`idItem`,`idLanguage`,`sTranslator`,`sTitle`,`sImageUrl`,`sSubtitle`,`sDescription`,`sEduComment`,`sRankingComment`)       VALUES (NEW.`ID`,@curVersion,NEW.`idItem`,NEW.`idLanguage`,NEW.`sTranslator`,NEW.`sTitle`,NEW.`sImageUrl`,NEW.`sSubtitle`,NEW.`sDescription`,NEW.`sEduComment`,NEW.`sRankingComment`) ; END IF; END
-- +migrate StatementEnd
DROP TRIGGER `before_delete_items_strings`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_delete_items_strings` BEFORE DELETE ON `items_strings` FOR EACH ROW BEGIN SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion; UPDATE `history_items_strings` SET `iNextVersion` = @curVersion WHERE `ID` = OLD.`ID` AND `iNextVersion` IS NULL; INSERT INTO `history_items_strings` (`ID`,`iVersion`,`idItem`,`idLanguage`,`sTranslator`,`sTitle`,`sImageUrl`,`sSubtitle`,`sDescription`,`sEduComment`,`sRankingComment`, `bDeleted`) VALUES (OLD.`ID`,@curVersion,OLD.`idItem`,OLD.`idLanguage`,OLD.`sTranslator`,OLD.`sTitle`,OLD.`sImageUrl`,OLD.`sSubtitle`,OLD.`sDescription`,OLD.`sEduComment`,OLD.`sRankingComment`, 1); END
-- +migrate StatementEnd
DROP TRIGGER `before_insert_languages`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_languages` BEFORE INSERT ON `languages` FOR EACH ROW BEGIN IF (NEW.ID IS NULL OR NEW.ID = 0) THEN SET NEW.ID = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.iVersion = @curVersion; END
-- +migrate StatementEnd
DROP TRIGGER `after_insert_languages`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `after_insert_languages` AFTER INSERT ON `languages` FOR EACH ROW BEGIN INSERT INTO `history_languages` (`ID`,`iVersion`,`sName`,`sCode`) VALUES (NEW.`ID`,@curVersion,NEW.`sName`,NEW.`sCode`); END
-- +migrate StatementEnd
DROP TRIGGER `before_update_languages`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_update_languages` BEFORE UPDATE ON `languages` FOR EACH ROW BEGIN IF NEW.iVersion <> OLD.iVersion THEN SET @curVersion = NEW.iVersion; ELSE SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion; END IF; IF NOT (OLD.`ID` = NEW.`ID` AND OLD.`sName` <=> NEW.`sName` AND OLD.`sCode` <=> NEW.`sCode`) THEN   SET NEW.iVersion = @curVersion;   UPDATE `history_languages` SET `iNextVersion` = @curVersion WHERE `ID` = OLD.`ID` AND `iNextVersion` IS NULL;   INSERT INTO `history_languages` (`ID`,`iVersion`,`sName`,`sCode`)       VALUES (NEW.`ID`,@curVersion,NEW.`sName`,NEW.`sCode`) ; END IF; END
-- +migrate StatementEnd
DROP TRIGGER `before_delete_languages`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_delete_languages` BEFORE DELETE ON `languages` FOR EACH ROW BEGIN SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion; UPDATE `history_languages` SET `iNextVersion` = @curVersion WHERE `ID` = OLD.`ID` AND `iNextVersion` IS NULL; INSERT INTO `history_languages` (`ID`,`iVersion`,`sName`,`sCode`, `bDeleted`) VALUES (OLD.`ID`,@curVersion,OLD.`sName`,OLD.`sCode`, 1); END
-- +migrate StatementEnd
DROP TRIGGER `before_insert_messages`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_messages` BEFORE INSERT ON `messages` FOR EACH ROW BEGIN IF (NEW.ID IS NULL OR NEW.ID = 0) THEN SET NEW.ID = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.iVersion = @curVersion; END
-- +migrate StatementEnd
DROP TRIGGER `after_insert_messages`;
-- +migrate StatementBegin
...
DROP TRIGGER `before_update_messages`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_update_messages` BEFORE UPDATE ON `messages` FOR EACH ROW BEGIN IF NEW.iVersion <> OLD.iVersion THEN SET @curVersion = NEW.iVersion; ELSE SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion; END IF; IF NOT (OLD.`ID` = NEW.`ID` AND OLD.`idThread` <=> NEW.`idThread` AND OLD.`idUser` <=> NEW.`idUser` AND OLD.`sSubmissionDate` <=> NEW.`sSubmissionDate` AND OLD.`bPublished` <=> NEW.`bPublished` AND OLD.`sTitle` <=> NEW.`sTitle` AND OLD.`sBody` <=> NEW.`sBody` AND OLD.`bTrainersOnly` <=> NEW.`bTrainersOnly` AND OLD.`bArchived` <=> NEW.`bArchived` AND OLD.`bPersistant` <=> NEW.`bPersistant`) THEN   SET NEW.iVersion = @curVersion;   UPDATE `history_messages` SET `iNextVersion` = @curVersion WHERE `ID` = OLD.`ID` AND `iNextVersion` IS NULL;   INSERT INTO `history_messages` (`ID`,`iVersion`,`idThread`,`idUser`,`sSubmissionDate`,`bPublished`,`sTitle`,`sBody`,`bTrainersOnly`,`bArchived`,`bPersistant`)       VALUES (NEW.`ID`,@curVersion,NEW.`idThread`,NEW.`idUser`,NEW.`sSubmissionDate`,NEW.`bPublished`,NEW.`sTitle`,NEW.`sBody`,NEW.`bTrainersOnly`,NEW.`bArchived`,NEW.`bPersistant`) ; END IF; END
-- +migrate StatementEnd
DROP TRIGGER `before_delete_messages`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_delete_messages` BEFORE DELETE ON `messages` FOR EACH ROW BEGIN SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion; UPDATE `history_messages` SET `iNextVersion` = @curVersion WHERE `ID` = OLD.`ID` AND `iNextVersion` IS NULL; INSERT INTO `history_messages` (`ID`,`iVersion`,`idThread`,`idUser`,`sSubmissionDate`,`bPublished`,`sTitle`,`sBody`,`bTrainersOnly`,`bArchived`,`bPersistant`, `bDeleted`) VALUES (OLD.`ID`,@curVersion,OLD.`idThread`,OLD.`idUser`,OLD.`sSubmissionDate`,OLD.`bPublished`,OLD.`sTitle`,OLD.`sBody`,OLD.`bTrainersOnly`,OLD.`bArchived`,OLD.`bPersistant`, 1); END
-- +migrate StatementEnd
DROP TRIGGER `before_insert_threads`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_threads` BEFORE INSERT ON `threads` FOR EACH ROW BEGIN IF (NEW.ID IS NULL OR NEW.ID = 0) THEN SET NEW.ID = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.iVersion = @curVersion; END
-- +migrate StatementEnd
DROP TRIGGER `after_insert_threads`;
-- +migrate StatementBegin
...
DROP TRIGGER `before_insert_users`;
-- +migrate StatementBegin
CREATE DEFINER=`algorea`@`%` TRIGGER `before_insert_users` BEFORE INSERT ON `users` FOR EACH ROW BEGIN IF (NEW.ID IS NULL OR NEW.ID = 0) THEN SET NEW.ID = FLOOR(RAND() * 1000000000) + FLOOR(RAND() * 1000000000) * 1000000000; END IF ; SELECT (UNIX_TIMESTAMP() * 10) INTO @curVersion;SET NEW.iVersion = @curVersion; END
-- +migrate StatementEnd
DROP TRIGGER `after_insert_users`;
-- +migrate StatementBegin
