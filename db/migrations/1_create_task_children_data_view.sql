...
CREATE ALGORITHM=UNDEFINED
  ...
  SELECT
    ...
    SUM(IF(((`items_items`.`sCategory` = 'Validation') AND
      (ISNULL(`task_children`.`ID`) OR (`task_children`.`bValidated` = 0))), 1, 0)) AS `nbChildrenCategory`,
  ...
  FROM (
    (
      (`users_items` `parent_users_items`
       JOIN `items_items` ON((`parent_users_items`.`idItem` = `items_items`.`idItemParent`)))
      LEFT JOIN `users_items` `task_children`
      ON(((`items_items`.`idItemChild` = `task_children`.`idItem`) AND
