...
CREATE DEFINER=`algorea`@`%` TRIGGER `after_insert_items_items` AFTER INSERT ON `items_items` FOR EACH ROW BEGIN
    INSERT INTO `history_items_items` (`id`,`version`,`item_parent_id`,`item_child_id`,`child_order`,`category`,
                                       `partial_access_propagation`,`difficulty`)
        VALUES (NEW.`id`,@curVersion,NEW.`item_parent_id`,NEW.`item_child_id`,NEW.`child_order`,NEW.`category`,
    ...
    INSERT IGNORE INTO `groups_items_propagate`
        SELECT `id`, 'children' as `propagate_access` FROM `groups_items`
            WHERE `groups_items`.`item_id` = NEW.`item_parent_id`
...
CREATE DEFINER=`algorea`@`%` TRIGGER `before_update_items_items` BEFORE UPDATE ON `items_items` FOR EACH ROW BEGIN
    ...
    END IF;
    IF NOT (OLD.`id` = NEW.`id` AND OLD.`item_parent_id` <=> NEW.`item_parent_id` AND
            OLD.`item_child_id` <=> NEW.`item_child_id` AND OLD.`child_order` <=> NEW.`child_order` AND
        ...
        INSERT INTO `history_items_items` (`id`,`version`,`item_parent_id`,`item_child_id`,`child_order`,`category`,
            ...
            VALUES (NEW.`id`,@curVersion,NEW.`item_parent_id`,NEW.`item_child_id`,NEW.`child_order`,NEW.`category`,
                    NEW.`partial_access_propagation`,NEW.`difficulty`);
    END IF;
    IF (OLD.item_child_id != NEW.item_child_id OR OLD.item_parent_id != NEW.item_parent_id) THEN
        INSERT IGNORE INTO `items_propagate` (`id`, `ancestors_computation_state`)
            VALUES (OLD.item_child_id, 'todo') ON DUPLICATE KEY UPDATE `ancestors_computation_state` = 'todo';
        INSERT IGNORE INTO `items_propagate` (`id`, `ancestors_computation_state`)
            VALUES (OLD.item_parent_id, 'todo') ON DUPLICATE KEY UPDATE `ancestors_computation_state` = 'todo';
        INSERT IGNORE INTO `items_propagate` (`id`, `ancestors_computation_state`)
            (
                SELECT `items_ancestors`.`item_child_id`, 'todo'
                FROM `items_ancestors`
                    WHERE `items_ancestors`.`item_ancestor_id` = OLD.`item_child_id`
            ) ON DUPLICATE KEY UPDATE `ancestors_computation_state` = 'todo';
        DELETE `items_ancestors` from `items_ancestors`
            WHERE `items_ancestors`.`item_child_id` = OLD.`item_child_id` AND
                  `items_ancestors`.`item_ancestor_id` = OLD.`item_parent_id`;
        DELETE `bridges` FROM `items_ancestors` `child_descendants`
            ...
            JOIN `items_ancestors` `bridges` ON (
                `bridges`.`item_ancestor_id` = `parent_ancestors`.`item_ancestor_id` AND
                `bridges`.`item_child_id` = `child_descendants`.`item_child_id`
            )
            WHERE `parent_ancestors`.`item_child_id` = OLD.`item_parent_id` AND
                  `child_descendants`.`item_ancestor_id` = OLD.`item_child_id`;
        DELETE `child_ancestors` FROM `items_ancestors` `child_ancestors`
            JOIN  `items_ancestors` `parent_ancestors` ON (
                `child_ancestors`.`item_child_id` = OLD.`item_child_id` AND
                `child_ancestors`.`item_ancestor_id` = `parent_ancestors`.`item_ancestor_id`
            )
            WHERE `parent_ancestors`.`item_child_id` = OLD.`item_parent_id`;
        DELETE `parent_ancestors` FROM `items_ancestors` `parent_ancestors`
            JOIN  `items_ancestors` `child_ancestors` ON (
                `parent_ancestors`.`item_ancestor_id` = OLD.`item_parent_id` AND
                `child_ancestors`.`item_child_id` = `parent_ancestors`.`item_child_id`
            )
            WHERE `child_ancestors`.`item_ancestor_id` = OLD.`item_child_id`;
    END IF;
    IF (OLD.item_child_id != NEW.item_child_id OR OLD.item_parent_id != NEW.item_parent_id) THEN
        INSERT IGNORE INTO `items_propagate` (id, ancestors_computation_state)
            VALUES (NEW.item_child_id, 'todo') ON DUPLICATE KEY UPDATE `ancestors_computation_state` = 'todo';
    END IF;
END
-- +migrate StatementEnd
...
CREATE DEFINER=`algorea`@`%` TRIGGER `before_delete_items_items` BEFORE DELETE ON `items_items` FOR EACH ROW BEGIN
    ...
    INSERT INTO `history_items_items` (`id`,`version`,`item_parent_id`,`item_child_id`,`child_order`,`category`,
        ...
        VALUES (OLD.`id`,@curVersion,OLD.`item_parent_id`,OLD.`item_child_id`,OLD.`child_order`,OLD.`category`,
    ...
    INSERT IGNORE INTO `items_propagate` (`id`, `ancestors_computation_state`)
        (
            SELECT `items_ancestors`.`item_child_id`, 'todo' FROM `items_ancestors`
                WHERE `items_ancestors`.`item_ancestor_id` = OLD.`item_child_id`
        ) ON DUPLICATE KEY UPDATE `ancestors_computation_state` = 'todo';
    DELETE `items_ancestors` FROM `items_ancestors`
        WHERE `items_ancestors`.`item_child_id` = OLD.`item_child_id` AND
    ...
    INSERT IGNORE INTO `groups_items_propagate`
        SELECT `id`, 'children' as `propagate_access` FROM `groups_items`
            WHERE `groups_items`.`item_id` = OLD.`item_parent_id`
        ON DUPLICATE KEY UPDATE propagate_access='children';
END
-- +migrate StatementEnd
