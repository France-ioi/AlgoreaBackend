...
func TestSessionStore_CreateNewTempSession(t *testing.T) {
	...
	mock.ExpectExec("^"+regexp.QuoteMeta(
		"INSERT INTO `sessions` (idUser, sAccessToken, sExpirationDate) VALUES (?, ?, NOW() + INTERVAL ? SECOND)",
	)+"$").WithArgs(expectedUserID, expectedAccessToken, 2*60*60).
...
func TestSessionStore_CreateNewTempSession_Retries(t *testing.T) {
	...
	mock.ExpectExec("^"+regexp.QuoteMeta(
		"INSERT INTO `sessions` (idUser, sAccessToken, sExpirationDate) VALUES (?, ?, NOW() + INTERVAL ? SECOND)",
	)+"$").WithArgs(expectedUserID, expectedAccessTokens[0], 2*60*60).
		WillReturnError(
			&mysql.MySQLError{
				Number:  1062,
	...
	mock.ExpectExec("^"+regexp.QuoteMeta(
		"INSERT INTO `sessions` (idUser, sAccessToken, sExpirationDate) VALUES (?, ?, NOW() + INTERVAL ? SECOND)",
	)+"$").WithArgs(expectedUserID, expectedAccessTokens[1], 2*60*60).
		WillReturnResult(sqlmock.NewResult(1, 1))
