...
func (s *UserItemStore) PropagateAttempts() (err error) {
	...
	mustNotBeError(s.db.Exec(`
		...
		JOIN groups_groups ON groups_groups.group_parent_id = groups_attempts.group_id AND
			groups_groups.type` + GroupRelationIsActiveCondition + `
		JOIN users ON users.id = users_items.user_id AND users.group_self_id = groups_groups.group_child_id
	...
	mustNotBeError(s.db.Exec(`
		...
		JOIN (
			...
			JOIN groups_groups AS attempt_group
				ON attempts.group_id = attempt_group.group_parent_id AND attempt_user.group_self_id = attempt_group.group_child_id AND
