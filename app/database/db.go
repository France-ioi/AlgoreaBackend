...
var _ gorm.StatementModifier = &suppressFromClause{}
var _ clause.Expression = &suppressFromClause{}
...
var _ gorm.StatementModifier = &suppressSelectClause{}
var _ clause.Expression = &suppressSelectClause{}
...
func (conn *DB) New() *DB {
	return newDB(conn.db.Session(&gorm.Session{
		NewDB: true,
	}))
}
...
func (conn *DB) handleDeadLock(txFunc func(*DB) error, count int64, errToHandle interface{},
	...
	if err, ok := errToHandle.(error); ok {
		if ok := errors.As(err, &e); ok && e.Number == 1213 {
			if rollbackErr != nil {
				panic(rollbackErr)
			}
			// retry
...
func (conn *DB) Limit(limit int) *DB {
	return newDB(conn.db.Limit(limit).Session(&gorm.Session{}))
}
...
func (conn *DB) Where(query interface{}, args ...interface{}) *DB {
	return newDB(conn.db.Where(query, args...).Session(&gorm.Session{}))
}

// Joins specifies Joins conditions
//     db.Joins("JOIN emails ON emails.user_id = users.id AND emails.email = ?", "jinzhu@example.org").Find(&user)
func (conn *DB) Joins(query string, args ...interface{}) *DB {
	return newDB(conn.db.Joins(query, args...).Session(&gorm.Session{}))
}
...
func (conn *DB) Or(query interface{}, args ...interface{}) *DB {
	return newDB(conn.db.Or(query, args...).Session(&gorm.Session{}))
}
...
func (conn *DB) Table(name string, args ...interface{}) *DB {
	return newDB(conn.db.Table(name, args...).Session(&gorm.Session{}))
}

// Group specifies the group method on the find
func (conn *DB) Group(query string) *DB {
	return newDB(conn.db.Group(query).Session(&gorm.Session{}))
}
...
func (conn *DB) Order(value interface{}) *DB {
	return newDB(conn.db.Order(value).Session(&gorm.Session{}))
}
...
func (conn *DB) Having(query interface{}, args ...interface{}) *DB {
	return newDB(conn.db.Having(query, args...).Session(&gorm.Session{}))
}
...
func (conn *DB) Raw(query string, args ...interface{}) *DB {
	...
	return newDB(conn.db.Raw(query, args...).Session(&gorm.Session{}))
}
...
func (conn *DB) Scan(dest interface{}) *DB {
	result := newDB(conn.db.Scan(dest))
...
func (conn *DB) Take(dest interface{}, conds ...interface{}) *DB {
	return newDB(conn.db.Take(dest, conds...))
}
