...
func TestItemStore_ContestManagedByUser(t *testing.T) {
	...
	dbMock.ExpectQuery(regexp.QuoteMeta("SELECT items.id FROM `items` "+
		...
		"HAVING (MAX(can_grant_view_generated_value) >= ?)"+
		") AS permissions ON permissions.item_id = items.id "+
		"WHERE (items.id = ?) AND (items.duration IS NOT NULL) AND (can_watch_generated_value >= ?) LIMIT 1")).
		WithArgs(int64(2), 2, int64(123), 2).WillReturnRows(dbMock.NewRows([]string{"id"}).AddRow(123))
	var id int64
