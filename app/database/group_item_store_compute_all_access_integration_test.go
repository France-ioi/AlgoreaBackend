...
func TestGroupItemStore_ComputeAllAccess_Concurrency(t *testing.T) {
	...
	assert.NoError(t, groupItemStore.Order("group_id, item_id").
		Select("group_id, item_id, 'done' AS propagate_access").Scan(&result).Error())
	...
	assert.NoError(t, groupItemStore.Table("groups_items_propagate").Joins("LEFT JOIN groups_items USING(id)").
		Order("group_id, item_id").Select("group_id, item_id, propagate_access").Scan(&result).Error())
	assert.Equal(t, allDone, result)
}
