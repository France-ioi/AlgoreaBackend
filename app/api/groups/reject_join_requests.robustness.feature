Feature: Reject group requests - robustness
  ...
  Scenario: Fails when the parent group id is wrong
    ...
    When I send a POST request to "/groups/abc/requests/reject?group_ids=31,141,21,11,13"
  ...
  Scenario: Fails when group_ids is wrong
    Given I am the user with id "21"
    When I send a POST request to "/groups/13/requests/reject?group_ids=31,abc,11,13"
    Then the response code should be 400
