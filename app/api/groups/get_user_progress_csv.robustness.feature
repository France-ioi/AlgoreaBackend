Feature: Export the current progress of users on a subset of items as CSV (groupUserProgressCSV) - robustness
  Background:
  ...
  Scenario: User is not able to watch group members
    ...
    When I send a GET request to "/groups/13/user-progress-csv?parent_item_ids=210"
    Then the response code should be 403
    And the response error message should contain "Insufficient access rights"

  Scenario: Group id is incorrect
    Given I am the user with id "21"
    When I send a GET request to "/groups/abc/user-progress-csv?parent_item_ids=210"
    Then the response code should be 400
    And the response error message should contain "Wrong value for group_id (should be int64)"

  Scenario: parent_item_ids is incorrect
    Given I am the user with id "21"
    When I send a GET request to "/groups/13/user-progress-csv?parent_item_ids=abc,123"
    Then the response code should be 400
  ...
  Scenario: Not enough permissions to watch results on parent_item_ids
    Given I am the user with id "21"
    When I send a GET request to "/groups/13/user-progress-csv?parent_item_ids=210,211"
    Then the response code should be 403
    And the response error message should contain "Insufficient access rights"
