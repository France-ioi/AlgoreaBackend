Feature: Remove members from a group (groupRemoveMembers)
  ...
  Scenario: Fails when the user is not an owner of the parent group
    ...
    When I send a DELETE request to "/groups/13/members?user_group_ids=1,2"
  ...
  Scenario: Fails when the parent group id is wrong
    ...
    When I send a DELETE request to "/groups/abc/members?user_group_ids=1,2"
    Then the response code should be 400
    And the response error message should contain "Wrong value for group_id (should be int64)"
    And the table "groups_groups" should stay unchanged
    And the table "groups_ancestors" should stay unchanged

  Scenario: Fails when user_group_ids is wrong
    Given I am the user with group_id "21"
    When I send a DELETE request to "/groups/13/members?user_group_ids=1,abc,2"
    Then the response code should be 400
    And the response error message should contain "Unable to parse one of the integers given as query args (value: 'abc', param: 'user_group_ids')"
    And the table "groups_groups" should stay unchanged
    And the table "groups_ancestors" should stay unchanged
