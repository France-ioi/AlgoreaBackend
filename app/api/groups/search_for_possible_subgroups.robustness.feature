Feature: Search for possible subgroups - robustness
  Background:
  ...
  Scenario: Should fail if the search string is not present
    ...
    When I send a GET request to "/groups/possible-subgroups"
  ...
  Scenario: Should fail if the search string is too small (search for "  中国  ")
    Given I am the user with id "21"
    When I send a GET request to "/groups/possible-subgroups?search=%20%20%E4%B8%AD%E5%9B%BD%20%20"
    Then the response code should be 400
  ...
  Scenario: Should fail if the user doesn't exist
    ...
    When I send a GET request to "/groups/possible-subgroups?search=abcdef"
    Then the response code should be 401
    And the response error message should contain "Invalid access token"

  Scenario: sort is incorrect
    Given I am the user with id "21"
    When I send a GET request to "/groups/possible-subgroups?search=abcdef&sort=myname"
    Then the response code should be 400
