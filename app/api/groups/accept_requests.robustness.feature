Feature: Accept group requests - robustness
  ...
  Scenario: Fails when the user is not an owner of the parent group
    ...
    When I send a POST request to "/groups/13/accept_requests?group_ids=31,141,21,11,13,22"
  ...
  Scenario: Fails when the parent group ID is wrong
    ...
    When I send a POST request to "/groups/abc/accept_requests?group_ids=31,141,21,11,13,22"
  ...
  Scenario: Fails when group_ids is wrong
    Given I am the user with ID "1"
    When I send a POST request to "/groups/13/accept_requests?group_ids=31,abc,11,13,22"
    Then the response code should be 400
