Feature: Invite users
  Background:
    ...
    And the database has the following table 'groups_groups':
      | parent_group_id | child_group_id | type   | status_changed_at |
      | 22              | 13             | direct | null              |
  ...
  Scenario: Successfully invite users
    ...
    And the table "groups_groups" should be:
      | parent_group_id | child_group_id | type           | role   | inviting_user_id | child_order = 0 | (status_changed_at IS NOT NULL) AND (ABS(TIMESTAMPDIFF(SECOND, status_changed_at, NOW())) < 3) |
