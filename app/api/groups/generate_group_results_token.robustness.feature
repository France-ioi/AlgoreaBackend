Feature: Generate a group results token (groupResultsTokenGenerate) - robustness
  ...
  Scenario: The number of items exceeds the limit
    ...
    And the database has the following table "permissions_generated":
      ...
      | 21 | 6099 | info | none |
      | 21 | 6100 | info | none |
    When I send a POST request to "/groups/11/group-results-token?parent_item_ids=6000"
    Then the response code should be 400
  ...
  Scenario: The number of users exceeds the limit
    ...
    And the database has the following table "permissions_generated":
      | group_id | item_id | can_view_generated | can_watch_generated |
      | 21       | 800     | info               | answer              |
    When I send a POST request to "/groups/11/group-results-token?parent_item_ids=800"
    Then the response code should be 400
