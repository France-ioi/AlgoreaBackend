...
Background:
  ...
  Scenario: Invalid item_id
    ...
    When I send a GET request to "/items/abc/children?attempt_id=0"
    Then the response code should be 400
    And the response error message should contain "Wrong value for item_id (should be int64)"

  Scenario: Should fail when attempt_id is invalid
    Given I am the user with id "11"
    When I send a GET request to "/items/200/children?as_team_id=15&attempt_id=abc"
    Then the response code should be 400
  ...
  Scenario: Should fail when the current user is not a member of as_team_id
    Given I am the user with id "11"
    When I send a GET request to "/items/200/children?as_team_id=14&attempt_id=0"
    Then the response code should be 403
    And the response error message should contain "Can't use given as_team_id as a user's team"

  Scenario: Should fail when the user doesn't exist
    Given I am the user with id "404"
    When I send a GET request to "/items/190/children?attempt_id=0"
    Then the response code should be 401
    And the response error message should contain "Invalid access token"

  Scenario: Should fail when the user doesn't have content access to the root item
    Given I am the user with id "11"
    When I send a GET request to "/items/190/children?attempt_id=0"
    Then the response code should be 403
  ...
  Scenario: Should fail when the root item doesn't exist
    Given I am the user with id "11"
    When I send a GET request to "/items/404/children?attempt_id=0"
    Then the response code should be 403
    And the response error message should contain "Insufficient access rights"

  Scenario: Should fail when the team doesn't have content access to the root item
    Given I am the user with id "11"
    When I send a GET request to "/items/200/children?as_team_id=15&attempt_id=0"
    Then the response code should be 403
    And the response error message should contain "Insufficient access rights"
  ...
  Scenario: Should fail when there is no started result for the item and attempt_id
    Given I am the user with id "11"
    When I send a GET request to "/items/210/children?as_team_id=15&attempt_id=1"
    Then the response code should be 403
    And the response error message should contain "Insufficient access rights"

  Scenario: Should fail when watched_group_id is invalid
    Given I am the user with id "11"
    When I send a GET request to "/items/210/children?as_team_id=15&attempt_id=2&watched_group_id=abc"
    Then the response code should be 400
