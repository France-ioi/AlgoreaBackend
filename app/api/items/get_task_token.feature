Feature: Get a task token with a refreshed attempt for an item
  ...
  Scenario: User is able to fetch a task token
    ...
    And the database has the following table 'groups_attempts':
      | id | group_id | item_id | order | latest_activity_at  | started_at | score_computed | best_answer_id | validated_at | hints_requested | hints_cached |
    ...
    And the table "groups_attempts" at id "2" should be:
      | id | group_id | item_id | score_computed | tasks_tried | result_propagation_state | ABS(TIMESTAMPDIFF(SECOND, latest_activity_at, NOW())) < 3 | ABS(TIMESTAMPDIFF(SECOND, latest_answer_at, NOW())) < 3 | ABS(TIMESTAMPDIFF(SECOND, validated_at, NOW())) < 3 | ABS(TIMESTAMPDIFF(SECOND, started_at, NOW())) < 3 |
      | 2  | 101      | 50      | 0              | 0           | done                     | 1                                                         | null                                                    | null                                                | 1                                                 |
  ...
  Scenario: User is able to fetch a task token as a team
    ...
    And the table "groups_attempts" at id "2" should be:
      | id | group_id | item_id | score_computed | tasks_tried | result_propagation_state | ABS(TIMESTAMPDIFF(SECOND, latest_activity_at, NOW())) < 3 | ABS(TIMESTAMPDIFF(SECOND, latest_answer_at, NOW())) < 3 | ABS(TIMESTAMPDIFF(SECOND, validated_at, NOW())) < 3 | ABS(TIMESTAMPDIFF(SECOND, started_at, NOW())) < 3 |
      | 2  | 102      | 60      | 0              | 0           | done                     | 1                                                         | null                                                    | null                                                | 1                                                 |

  Scenario: Keeps previous started_at values
    Given I am the user with id "101"
    And the database has the following table 'groups_attempts':
      | id | group_id | item_id | order | latest_activity_at  | started_at          | score_computed | best_answer_id | validated_at |
    ...
    And the table "groups_attempts" should stay unchanged but the row with id "2"
    And the table "groups_attempts" at id "2" should be:
      | id | group_id | item_id | score_computed | tasks_tried | result_propagation_state | ABS(TIMESTAMPDIFF(SECOND, latest_activity_at, NOW())) < 3 | ABS(TIMESTAMPDIFF(SECOND, latest_answer_at, NOW())) < 3 | ABS(TIMESTAMPDIFF(SECOND, validated_at, NOW())) < 3 | started_at          |
      | 2  | 101      | 50      | 0              | 0           | done                     | 1                                                         | null                                                    | null                                                | 2017-05-29 06:38:38 |
