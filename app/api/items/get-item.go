...
func (srv *Service) getItem(rw http.ResponseWriter, httpReq *http.Request) service.APIError {
	...
	rawData, err := dbItems.NewStore(srv.Store).GetRawItemData(req.ID, user.UserID, user.DefaultLanguageID(), user)
	if err != nil {
...
func setItemResponseRootNodeFields(response *itemResponse, rawData *[]dbItems.RawItem) {
...
func (srv *Service) fillItemCommonFieldsWithDBData(rawData *dbItems.RawItem) *itemCommonFields {
...
func (srv *Service) fillItemResponseWithChildren(response *itemResponse, rawData *[]dbItems.RawItem) {
