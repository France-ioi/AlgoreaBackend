...
//
//   * Then `started_at` (if it is NULL) and `last_activity_at` of `groups_attempts` & `user_items` are set to the current time.
//
...
func (srv *Service) getTaskToken(w http.ResponseWriter, r *http.Request) service.APIError {
	...
	err = srv.Store.InTransaction(func(store *database.DataStore) error {
		...
		if activeAttemptID == nil {
			...
			err = groupAttemptScope.Order("last_activity_at DESC").
			...
			if gorm.IsRecordNotFoundError(err) {
				...
				service.MustNotBeError(err)
			} else { // otherwise, update groups_attempts.started_at (if it is NULL) & groups_attempts.last_activity_at
		...
		// update users_items.active_attempt_id, users_items.started_at (if it is NULL), and users_items.last_activity_at
		service.MustNotBeError(userItemStore.Where("user_id = ?", user.ID).Where("item_id = ?", itemID).
