Feature: Get root skills for a participant group - robustness
  ...
  Scenario: watched_group_id is invalid
    Given I am the user with id "11"
    When I send a GET request to "/current-user/group-memberships/skills?watched_group_id=abc"
    Then the response code should be 400
  ...
  Scenario: Both watched_group_id and as_team_id are given
    Given I am the user with id "11"
    When I send a GET request to "/current-user/group-memberships/skills?watched_group_id=13&as_team_id=14"
    Then the response code should be 400
