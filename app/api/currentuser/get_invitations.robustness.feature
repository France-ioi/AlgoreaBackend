Feature: Get group invitations for the current user - robustness
  ...
  Scenario: User doesn't exist
    ...
    When I send a GET request to "/current-user/invitations"
    Then the response code should be 403
    And the response error message should contain "Insufficient access rights"

  Scenario: within_weeks is incorrect
    Given I am the user with ID "1"
    When I send a GET request to "/current-user/invitations?within_weeks=abc"
  ...
  Scenario: sort is incorrect
    Given I am the user with ID "1"
    When I send a GET request to "/current-user/invitations?sort=myname"
    Then the response code should be 400
