Feature: User rejects an invitation - robustness
  Background:
  ...
  Scenario: User tries to reject an invitation that doesn't exist
    ...
    When I send a POST request to "/current-user/invitations/11/reject"
  ...
  Scenario: Fails when the group ID is wrong
    Given I am the user with ID "1"
    When I send a POST request to "/current-user/invitations/abc/reject"
  ...
  Scenario: Fails if the user doesn't exist
    ...
    When I send a POST request to "/current-user/invitations/13/reject"
