...
//   On success the service sets `groups_groups.type` to `left` and `status_date` to current UTC time.
...
//
//   * The user cannot leave the group if `NOW()` < `groups.lock_user_deletion_date`.
// parameters:
// - name: group_id
//   in: path
//   type: integer
