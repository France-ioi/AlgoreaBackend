Feature: User accepts an invitation - robustness
  Background:
  ...
  Scenario: User tries to create a cycle in the group relations graph
    ...
    When I send a POST request to "/current-user/invitations/13/accept"
  ...
  Scenario: Fails when the group ID is wrong
    Given I am the user with ID "1"
    When I send a POST request to "/current-user/invitations/abc/accept"
