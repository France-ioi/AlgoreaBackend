Feature: Create an access token
  ...
  Scenario: Ignores the cookie when both code and access_token cookie are present (while use_cookie=0)
    ...
    And the login module "account" endpoint for token "{{access_token_from_oauth}}" returns 200 with body:
      ...
      """
    When I send a POST request to "/auth/token?code=somecode&code_verifier=123456&redirect_uri=http%3A%2F%2Fmy.url"
    Then the response code should be 201
    And the response body should be, in JSON:
      """
      {
