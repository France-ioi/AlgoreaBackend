...
func (srv *Service) constructListThreadsQuery(httpRequest *http.Request, params listThreadParameters) (*database.DB, error) {
	...
	queryDB := query.
		...
		Select(`
			...
			IF(threads.participant_id = ? OR personal_info_view_approvals.approved, users.profile_first_name, NULL) AS participant__first_name,
			IF(threads.participant_id = ? OR personal_info_view_approvals.approved, users.profile_last_name, NULL) AS participant__last_name,
