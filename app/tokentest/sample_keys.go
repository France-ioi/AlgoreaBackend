...
const AlgoreaPlatformPublicKey = "" +
	"-----BEGIN PUBLIC KEY-----\n" +
...
// AlgoreaPlatform tokens in tests.
func AlgoreaPlatformPublicKeyParsed() *rsa.PublicKey {
	key, _ := crypto.ParseRSAPublicKeyFromPEM([]byte(AlgoreaPlatformPublicKey))
	return key
}

// AlgoreaPlatformPrivateKey represents a sample private key used to encode
// AlgoreaPlatform tokens in tests.
const AlgoreaPlatformPrivateKey = "" +
...
// AlgoreaPlatform tokens in tests.
func AlgoreaPlatformPrivateKeyParsed() *rsa.PrivateKey {
	key, _ := crypto.ParseRSAPrivateKeyFromPEM([]byte(AlgoreaPlatformPrivateKey))
	return key
}

// TaskPlatformPublicKey represents a sample public key used to decode
// TaskPlatform tokens in tests.
const TaskPlatformPublicKey = "" +
	"-----BEGIN PUBLIC KEY-----\n" +
...
// TaskPlatform tokens in tests.
func TaskPlatformPrivateKeyParsed() *rsa.PrivateKey {
	key, _ := crypto.ParseRSAPrivateKeyFromPEM([]byte(TaskPlatformPrivateKey))
	return key
}
