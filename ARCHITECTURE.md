...
- **CORS**: Configured in `app/cors.go` from the top-level `cors` section:
  - `cors.allowedOrigins`: full origins (scheme + host + optional port). Supports the bare `*` wildcard and per-host wildcards like `https://*.example.com`. Defaults to `[]` in every environment when unset (no env-based bypass). **Note**: `go-chi/cors@v1.0.0` treats an empty list identically to `["*"]` (sets `allowedOriginsAll = true`), so with the default `allowCredentials=false` the middleware still echoes any caller's `Origin` back without `Access-Control-Allow-Credentials` -- any site can read non-credentialed responses by default. Fail-closed behavior only applies to the credentialed path: see the safety rule below.
  - `cors.allowCredentials`: whether the middleware sends `Access-Control-Allow-Credentials: true`. Defaults to `false`.
  - Startup safety rule: when `allowCredentials` is `true`, `allowedOrigins` MUST be an explicit non-empty list that does not contain the bare `"*"`. Both an unset/empty list and a list containing `"*"` are rejected, because `go-chi/cors@v1.0.0` treats `len(AllowedOrigins) == 0` identically to `["*"]` (sets `allowedOriginsAll = true`); either way the middleware would echo any caller's `Origin` back together with `Access-Control-Allow-Credentials: true` -- the canonical CSRF-via-CORS misconfiguration. The "credentials-only" config (only `allowCredentials: true` set, e.g. via the `ALGOREA_CORS__ALLOWCREDENTIALS` env var) is therefore also caught at startup. Per-host wildcards (`https://*.example.com`) are not affected and remain allowed alongside credentials.
  - **Migration note**: the previous implementation hardcoded `AllowedOrigins: ["*"]` and `AllowCredentials: true`, and the first iteration of the configurable version kept `["*"]` as a dev/test fallback when the key was unset. Both of those are now gone -- the unset case resolves to `[]` + `allowCredentials=false` in **every** environment, including `dev` and `test`. Effective consequences for existing setups: any local dev frontend (or browser-driven integration test) that relied on credentialed cross-origin requests against the backend without a `cors:` block will silently stop receiving `Access-Control-Allow-Credentials: true`. To restore the prior behavior, add an explicit `cors:` block with the trusted origins and `allowCredentials: true`. The repo's own BDD/Cucumber suite invokes the chi handler in-process and does not exercise CORS, so it is unaffected.
